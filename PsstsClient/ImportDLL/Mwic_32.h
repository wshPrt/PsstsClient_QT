extern "C"
{
	...
	例：
	crt_reset(icdev, 0x31);											*/
	int __stdcall crt_reset(HANDLE icdev, unsigned char _Ejct);
	...
	_EnableBackIn：是否允许后端进卡。 0x30=允许；0x31=不允许。
	CRT310 读卡器上电或执行复位命令后默认的前端进卡方式是开关方式进卡使能，后端允许进卡。

	成功返回0，失败返回-1;

	例：
	允许插卡：crt_card_setting(icdev, 0x33, 0x31);
	禁止插卡: crt_card_setting(icdev, 0x31, 0x31);								*/
	int __stdcall crt_card_setting(HANDLE icdev, unsigned char _CardIn, unsigned char _EnableBackIn);

	/**************************************************************************************/
	/*	走卡位置设置
	_Position： =0x2E 将卡重新走位到卡机内位置,操作成功后可进行 M1 射频卡操作。
	...
	例：
	吐卡：crt_moveposition(icdev, 0x31);
	移动卡片并使触电下落:crt_moveposition(icdev, 0x2f);							*/
	int __stdcall crt_moveposition(HANDLE icdev, unsigned char _position);

	/**************************************************************************************/
	/*	应用程序在操作接触式 IC 卡之前，需调用此函数给卡片上电
	成功返回0，失败返回-1;							*/
	int __stdcall crt_cardopen(HANDLE icdev);

	/**************************************************************************************/
	/*	应用程序在操作完接触式 IC 后，在用户拔出卡片之前，需调用此函数以给卡片下电。
	成功返回0，失败返回-1;											*/
	int __stdcall crt_cardclose(HANDLE icdev);
